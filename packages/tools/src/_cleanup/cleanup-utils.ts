...
export const makeMutators = (
  ...
  const _padBlockElements = (html: string) => {
  ...
  const prettify = (html: HTMLText) => {
    const paddedHtml = _padBlockElements(html)
      .trim()
      .replace(/&nbsp;/g, ' ');

    const PRE_PLACEHOLDER = '$$__PRE_PLACEHOLDER__$$';
    const prePlaceholderRe = new RegExp(
      PRE_PLACEHOLDER.replace(/\$/g, '\\$'),
      'g',
    );
    const pres: Array<string> = [];

    const htmlSansPre = paddedHtml.replace(
      /<pre[\s>][^]+?<\/pre\s*>/g,
      (preMatch) => {
        pres.push(preMatch);
    ...
    const prettifiedHtml = prettier
      ...
      .trimStart()
      .replace(prePlaceholderRe, () => pres.shift() || ''); // re-insert formatting <pre>s as is
  ...
  const removeUnwantedAttributes = (
    ...
    const allowedAttrs: Record<string, AllowedAttrs> = {
      _always: {
    ...
    allowedAttrs.P.align = allowedAttrs._otherBlock.align;
