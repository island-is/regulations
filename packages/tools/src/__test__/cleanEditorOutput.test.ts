...
const cleanHTML = (html: HTMLText) => prettify(cleanupSingleEditorOutput(html));
const cleanHTMLTwice = (html: HTMLText) => cleanHTML(cleanHTML(html));

// ---------------------------------------------------------------------------

[cleanHTML, cleanHTMLTwice].forEach((cleanerFn) => {
  runCleanupFiletests(cleanerFn, {
  ...
  runCleanupMicroTests(cleanerFn, {
